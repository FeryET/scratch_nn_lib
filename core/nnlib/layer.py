...
class InputLayer(Layer):
  ...
  def backward(self, layer_input, delta_next_layer):
    raise NotImplementedError("reset grad is not implemented for an InputLayer instance")
...
class DenseLayer(Layer):
  ...
  def _initialize_params(self):
    ...
    bias = self._weight_initializer(shape=(self.output_dim,))
  ...
  def forward(self, X, train=True):
    ...
    """
    out = self.activation(self.params["weights"].T @ X + self.params["bias"])
    return out
  
  def backward(self, layer_input, delta_next_layer, w_next_layer):
    if w_next_layer is None:
      delta = delta_next_layer * self.activation.derivative(params["outputs"])
    else:
      delta = ((w_next_layer).T @ delta_next_layer) * self.activation.derivative(self.params["outputs"])
